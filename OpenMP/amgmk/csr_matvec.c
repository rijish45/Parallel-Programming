              ...
              hypre_Vector    *y     )
{
   ...
   // struct timespec start_time, end_time; //hw4
    ...
    *-----------------------------------------------------------------------*/
    // fprintf(stderr, "before alpha is 0\n");
...
#pragma omp parallel for default(shared) private(i)
    ...
    * y = (beta/alpha)*y
    *-----------------------------------------------------------------------*/
   // fprintf(stderr, "before y = beta/alpha \n");
    
   temp = beta / alpha;

   if (temp != 1.0)
   {
     // fprintf(stderr, "y = beta / alpha \n"); // hw4 reachable
...
/* use rownnz pointer to do the A*x multiplication  when num_rownnz is smaller than num_rows */
   ...
   {
     fprintf(stderr, "y += A*x"); // hw4 APXY - unreachable
#pragma omp parallel for default(shared) private(i)
              ...
              int fpt )
{
   ...
   fprintf(stderr, "Calling timer stop\n");
   Timer_Stop("matvec-whole"); // hw4
   fprintf(stderr, "Calling timer print\n");
   Timer_Print();
   fprintf(stderr, "Called timer print\n");
